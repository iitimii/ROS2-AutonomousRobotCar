...
class SimpleParameter(Node):
    def __init__(self):
        ...
        self.declare_parameter('simple_str_param', 'Default Timi')
    ...
    def paramChangeCallback(self, params):
        ...
        for param in params:
            if param.name == 'simple_int_param' and param.type_ == Parameter.Type.INTEGER:
                self.get_logger().info(f'Param simple_int_param change! New value is {param.value}')
                result.successful = True

            if param.name == 'simple_string_param' and param.type_ == Parameter.Type.STRING:
                self.get_logger().info(f"Param simple_str_param change! New value is {param.value}")
                result.successful = True
