...
SimpleTfKinematics::SimpleTfKinematics(const std::string &node_name)
    : Node(node_name), x_increment_(0.05), last_x_(0.0), rotations_counter_(0)
{
...
bool SimpleTfKinematics::get_transform_callback_(const std::shared_ptr<GetTransform::Request> req, const std::shared_ptr<GetTransform::Response> res)
{
    ...
    {
        RCLCPP_ERROR_STREAM(get_logger(), "Failed to get transform from " << req->frame_id << "and " << req->child_frame_id << ": " << ex.what());
        res->success = false;
        return true;
    }
